 ...
 $insert="INSERT INTO doctors VALUES(null, '$name') ";  
   $i= mysqli_query($iconn ,$insert );
    ...
    if(isset( $_GET['edit'])){
        $updata=true;
        $id= $_GET['edit'];
        $select= "SELECT * FROM doctors where id=$id " ;// select into input
...
<?php if($updata) : ?>
    <h1 class="text-center text-info">Updata Doctors :<?php echo $id ?> </h1>
                <?php else: ?>
                    <h1 class="text-center text-info">Add Doctors Page</h1>
                <?php endif; ?>
